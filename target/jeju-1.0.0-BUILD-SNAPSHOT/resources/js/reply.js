	...
	let replyFunc = (() => {
		...
		function update(reply, callback){
			$.ajax({
				url: "/project01/replies/"+reply.rno,
				type: "put",
				data: JSON.stringify(reply),	// 문자열(텍스트) 변환 ==> 직렬화작업
				contentType: "application/json; charset=utf8",	// 직렬화 윗줄과 세트
				success: function(result){
...
/*		function getList(bid, callback){ // 페이징처리 이전코딩
			$.ajax({
				url: "/project01/replies/list/"+bid,
				type: "get",
				success: function(result){
					// (result) => {} 람다식으로도 가능
					console.log(result);
		...
		function getList(param, callback){
			...
			$.ajax({
				url: "/project01/replies/list/"+bid+"/"+viewPage,
				type: "get",
				success: function(result){
					// (result) => {} 람다식으로도 가능
					console.log(result);
